...
//	SpiffsUtil::saveSsid(server.arg("ssid").c_str());
	...
	if (m.length()==0)
		Serial.printf("not set as musical. Setting to 1.\n");
	else
		isMusical = atoi(m.c_str());
	Serial.printf("music fr spiffs:%s\n",m.c_str());

	FastLED.setBrightness(  BRIGHTNESS );
	currentPalette1 = RainbowColors_p;
	currentBlending = LINEARBLEND;

	product.addProperty(Universe, "0060", false, SymphProduct::createGui("1",SLIDER_OUT,"Set Universe",0, 10, myUniverse));
	product.addProperty(start, "0059", false, SymphProduct::createGui("2",RADIO_OUT,"Start",0, 1, isMusical));
	product.addProperty(stop, "0058", false, SymphProduct::createGui("2",RADIO_OUT,"Stop",0, 1, !isMusical));
	...
	s.on("/stop", handleStop);
	if (isMusical)
		e131.begin(E131_MULTICAST, myUniverse, 1);
    FastLED.addLeds<LED_TYPE, LED_PIN1, COLOR_ORDER>(leds, NUM_PIXELS).setCorrection( TypicalLEDStrip );
    FastLED.addLeds<LED_TYPE, LED_PIN2, COLOR_ORDER>(leds, NUM_PIXELS).setCorrection( TypicalLEDStrip );
...
/*
 * starts the musical mode
 */
void handleStart() {
	s.sendResponse("Will reboot and start as musical.");
	handleCommand(start, 0);
}

/*
 * stops the musical mode
 */
void handleStop() {
	s.sendResponse("Will reboot and start as stand-alone.");
...
void FillLEDsFromPaletteColors( uint8_t colorIndex)
{
    ...
    for( int i = 0; i < NUM_PIXELS; i++) {
        leds[i] = ColorFromPalette( currentPalette1, colorIndex, brightness, currentBlending);
        colorIndex += 3;
    }
}
