...
//		Vector<PropertyType> ptypes = new Vector<PropertyType>(10,1);
	...
	public String editProduct(@RequestParam(value="ssid", required=true) String ssid, 
		...
		args.put("SSID", ssid);
		UpdateDBEReq dber1 = new UpdateDBEReq(idg.generateERQSRequestID(), productTable, vals, args);
		try {
			dbe.forwardRequest(dber1, Thread.currentThread(), true);
	...
	public String createProduct(@RequestParam HashMap<String, String> requestParams, Model model) {
		...
		InsertDBEReq dber1 = new InsertDBEReq(idg.generateERQSRequestID(), productTable, prodVals);
		for(int i = 0; i < props.size(); i++) {
			...
			InsertDBEReq dber = new InsertDBEReq(idg.generateERQSRequestID(), comproplistTable, compropVals);
			compropReqs.add(dber);
		}
		try {
			dbe.forwardRequest(dber1, Thread.currentThread(), true);
			for(int i = 0; i < compropReqs.size(); i++) {
				dbe.forwardRequest(compropReqs.get(i), Thread.currentThread(), true);
			}
		} catch(EngineException e) {
	...
	public String editPropertyType(@RequestParam(value="ssid", required=true) String ssid, 
		...
		args.put("SSID", ssid);
		UpdateDBEReq dber1 = new UpdateDBEReq(idg.generateERQSRequestID(), pvalTable, vals, args);
		try {
			dbe.forwardRequest(dber1, Thread.currentThread(), true);
		} catch (EngineException e) {
	...
	public String createPropertyType(@RequestParam(value="ssid", required=true) String ssid, 
		...
		InsertDBEReq dber1 = new InsertDBEReq(idg.generateERQSRequestID(), pvalTable, values);
		try {
			dbe.forwardRequest(dber1, Thread.currentThread(), false);
		} catch (EngineException e) {
	...
	public String deletePropertyType(@RequestParam(value="ssid", required=true) String ssid, Model model) {
		...
		HashMap<String, Object> args = new HashMap<String, Object>(1);
		args.put("SSID", ssid);
		DeleteDBEReq dber1 = new DeleteDBEReq(idg.generateERQSRequestID(), pvalTable, args);
		try {
			dbe.forwardRequest(dber1, Thread.currentThread(), true);
