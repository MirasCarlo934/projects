...
void updateFW() {
      ...
      switch(ret) {
        case HTTP_UPDATE_FAILED:
          ...
          isUpdating = false;
          updateStatus = 203;
          /* we are not using this for the moment, need a better error message
          strcpy(firmwareUpdateMsg, "{\"resp\":\"0\",\"msg\":\"UPDATE_FAILED:");
          strcat(firmwareUpdateMsg,ESPhttpUpdate.getLastErrorString().c_str());
          strcat(firmwareUpdateMsg,"\"}");
          server.send(200, "text/html", firmwareUpdateMsg);*/
          server.send(200, "text/html", "{\"resp\":\"203\",\"msg\":\"UPDATE_FAILED\"}");
          break;
...
//			Serial.print("test lang ito name:");Serial.println(product.name);
			isMqttSuccessful = MqttUtil::connectToMQTT(myName.c_str(), mqttUrl.c_str(), 1883, &client,
					wifiClient, product);
...
//	}
	//	we generate the hostname based on the first 3 MAC values
	for (int i = 0; i < 3; ++i) {
			myName += String(mac[i], 16);
		}
	ap_ssid += myName;
